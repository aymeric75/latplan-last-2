...
"""
    def report(self,train_data,
        ...
        with open(self.local("perfs_"+self.parameters['label']+".txt"), "w") as f:
    ...
    def _load(self,path=""):
        ...
        try:
            with np.load(self.local(os.path.join(path,f"p_a_z0_net.npz"))) as data:
                print("loading the p_a_z_net files...")
...
class BaseActionMixinAMA4Plus(BidirectionalMixin, BaseActionMixin):
    ...
    def _build_primary(self,input_shape):
        ...
        if("invariants" in self.parameters):
            ...
            if( len(self.parameters["invariants"]) > 0 ):
                has_invariants = True
        ...
        if has_invariants:
            ...
            total_add_loss_norm = alpha*total_add_loss
            total_loss = total_loss + total_add_loss_norm


        # # MORE WEIGHT GIVEN TO SAE RECONSTRUCTION LOSS
        # x0y0*=2
        # x1y1*=2
